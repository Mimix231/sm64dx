...
all:
    ...
    install_name_tool -change $(BREW_PREFIX)/opt/glew/lib/libGLEW.2.2.dylib @executable_path/libGLEW.dylib $(APP_MACOS_DIR)/sm64dx; > /dev/null 2>&1 \
    ...
    codesign --force --deep --sign - $(APP_MACOS_DIR)/libGLEW.dylib; \
		...
		echo '    <string>sm64dx</string>' >> $(APP_CONTENTS_DIR)/Info.plist; \

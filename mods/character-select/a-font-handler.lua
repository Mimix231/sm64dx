--[[
    ...
    Use mxui_hud_add_font() to add fonts as shown in main.lua
]]
...
local mxui_classic_hud_get_font = mxui_hud_get_font
local mxui_classic_hud_set_font = mxui_hud_set_font
local mxui_classic_hud_print_text = mxui_hud_print_text
local mxui_classic_hud_print_text_interpolated = mxui_hud_print_text_interpolated
local mxui_classic_hud_measure_text = mxui_hud_measure_text
...
---@param scale integer
---@return DjuiFontType
function mxui_hud_add_font(texture, info, spacing, offset, backup, scale)
...
---@return nil
function mxui_hud_set_font(fontType)
    if fontType > FONT_COUNT then
        customFont = true
        customFontType = fontType
    else
        customFont = false
        mxui_classic_hud_set_font(fontType)
    end
end

---@return DjuiFontType
function mxui_hud_get_font()
    if customFont then
        return customFontType
    end
    return mxui_classic_hud_get_font()
end

local textShake = 0
function mxui_hud_effect_shake(intensity)
...
function mxui_hud_effect_wave(x, y, speed)
    textWaveX = x
    textWaveY = y
    textWaveSpeed = speed
end
...
---@param scale number
---@return nil
function mxui_hud_print_text(message, x, y, scale)
    if customFont then
        ...
        for i = 1, #message do
            ...
            if letter and letter ~= " " then
                ...
                mxui_hud_render_texture_tile(currFont.spritesheet,
        ...
        mxui_classic_hud_print_text(message, x, y, scale)
    end
end
...
function mxui_hud_print_text_interpolated(message, prevX, prevY, prevScale, x, y, scale)
    if customFont then
        if message == nil or message == "" then return end
        local message = string_to_table(message)
        local currFont = fontTable[customFontType]
        ...
        for i = 1, #message do
            ...
            if letter and letter ~= " " then
                ...
                mxui_hud_render_texture_tile_interpolated(currFont.spritesheet,
                prevX + xOffset,
                prevY + yOffset,
        ...
        mxui_classic_hud_print_text_interpolated(message, prevX, prevY, prevScale, x, y, scale)
    end
end

---@param message string
---@return number
function mxui_hud_measure_text(message)
    if customFont then
        if message == nil or message == "" then return end
        local message = string_to_table(message)
        local currFont = fontTable[customFontType]
        ...
        return mxui_classic_hud_measure_text(message)
    end
end
...
FONT_CHARACTERISTIC = mxui_hud_add_font(get_texture_info("char_select_font_characteristic"), fontdataCharacteristic, -5, 0, "X", 1)

...
local function character_resolve_model_name(modelName)
    if type(modelName) ~= TYPE_STRING or modelName == "" then
        return E_MODEL_ERROR_MODEL, nil
    end

    local normalized = modelName:gsub("\\", "/")
    normalized = normalized:match("([^/]+)$") or normalized
    normalized = normalized:gsub("%.geo%.bin$", "_geo")
    normalized = normalized:gsub("%.bin$", "")
    normalized = normalized:gsub("%.geo$", "_geo")

    local resolved = smlua_model_util_get_id(normalized)
    if resolved == E_MODEL_ERROR_MODEL and normalized:sub(-4) ~= "_geo" then
        normalized = normalized .. "_geo"
        resolved = smlua_model_util_get_id(normalized)
    end
    return resolved, normalized
end

local function character_resolve_model_info(modelInfo, fallbackModel)
    if type(modelInfo) == TYPE_INTEGER then
        if modelInfo ~= E_MODEL_ERROR_MODEL and modelInfo ~= E_MODEL_NONE then
            return modelInfo, nil, "model"
        end
        return fallbackModel, nil, "palette"
    end
    if type(modelInfo) == TYPE_STRING then
        local resolvedModel, resolvedName = character_resolve_model_name(modelInfo)
        if resolvedModel ~= E_MODEL_ERROR_MODEL and resolvedModel ~= E_MODEL_NONE then
            return resolvedModel, resolvedName, "model"
        end
        return fallbackModel, resolvedName or modelInfo, "model"
    end
    return fallbackModel, nil, "palette"
end

local function character_variant_metadata(name, modelInfo, variantType, variantSource)
    local variantIdSource = type(name) == TYPE_STRING and name or "Variant"
    if type(variantSource) == TYPE_STRING and variantSource ~= "" then
        variantIdSource = variantSource
    elseif type(modelInfo) == TYPE_STRING and modelInfo ~= "" then
        variantIdSource = modelInfo
    end
    return string_space_to_underscore(variantIdSource), variantType, variantSource
end
...
local function character_add(name, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
    ...
    local addedModel, resolvedVariantSource, variantType = character_resolve_model_info(modelInfo, E_MODEL_ERROR_MODEL)
    local variantId, storedVariantType, variantSource = character_variant_metadata(name, modelInfo, modelInfo ~= nil and variantType or "base", resolvedVariantSource)
...
local function character_add_costume(charNum, name, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
    ...
    local addedModel, resolvedVariantSource, variantType = character_resolve_model_info(modelInfo, tableCache.model)
    local variantId, storedVariantType, variantSource = character_variant_metadata(name, modelInfo, variantType, resolvedVariantSource)
...
local function character_edit_costume(charNum, charAlt, name, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
    ...
    local resolvedModel, resolvedVariantSource, variantType = character_resolve_model_info(modelInfo, tableCache.model)
    local variantId, storedVariantType, variantSource = character_variant_metadata(
        type(name) == TYPE_STRING and name or tableCache.name,
        modelInfo,
        modelInfo ~= nil and variantType or (tableCache.variantType or "palette"),
        modelInfo ~= nil and resolvedVariantSource or tableCache.variantSource
    )
    characterTable[charNum][charAlt] = characterTable[charNum][charAlt] and {
        ...
        variantId = modelInfo ~= nil and variantId or tableCache.variantId,
        variantType = modelInfo ~= nil and storedVariantType or tableCache.variantType,
        variantSource = modelInfo ~= nil and variantSource or tableCache.variantSource,
        baseChar = type(baseChar) == TYPE_INTEGER and baseChar or tableCache.baseChar,
        lifeIcon = (type(lifeIcon) == TYPE_TABLE or type(lifeIcon) == TYPE_TEX_INFO or type(lifeIcon) == TYPE_STRING) and lifeIcon or tableCache.lifeIcon,
        starIcon = tableCache.starIcon, -- Done to prevent it getting lost in the sauce
        camScale = type(camScale) == TYPE_INTEGER and camScale or tableCache.camScale,
...
---@note         mxui_hud_render_rect(x + (i * 17), y, 16, 16)
---@note     end
---@note end
---@note ```
...
local function character_add_variant(charNum, variantOrName, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
    if type(variantOrName) == TYPE_TABLE then
        local variant = variantOrName
        return character_add_costume(
            charNum,
            variant.name,
            variant.description,
            variant.credit,
            variant.color,
            variant.model or variant.modelInfo or variant.modelPath or variant.geo or variant.geoBin or variant.skin or variant.skinPath or variant.variantModel,
            variant.baseChar,
            variant.lifeIcon,
            variant.camScale
        )
    end
    return character_add_costume(charNum, variantOrName, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
end

local function character_edit_variant(charNum, charAlt, variantOrName, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
    if type(variantOrName) == TYPE_TABLE then
        local variant = variantOrName
        character_edit_costume(
            charNum,
            charAlt,
            variant.name,
            variant.description,
    ...
    character_edit_costume(charNum, charAlt, variantOrName, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
end

---@description Adds a DX-style model variant that can point at model ids, `_geo`, `_geo.bin`, or file-path based variants.
---@added sm64dx
local function character_add_model_variant(charNum, variantOrName, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
    return character_add_variant(charNum, variantOrName, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
end

---@description Edits a DX-style model variant that can point at model ids, `_geo`, `_geo.bin`, or file-path based variants.
---@added sm64dx
local function character_edit_model_variant(charNum, charAlt, variantOrName, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
    return character_edit_variant(charNum, charAlt, variantOrName, description, credit, color, modelInfo, baseChar, lifeIcon, camScale)
end

---@description Convenience helper for adding `_geo.bin`/`_geo` model variants with DX naming.
---@added sm64dx
local function character_add_geo_variant(charNum, variantOrName, description, credit, color, geoBin, baseChar, lifeIcon, camScale)
    if type(variantOrName) == TYPE_TABLE then
        return character_add_variant(charNum, variantOrName)
    end
    return character_add_variant(charNum, {
        name = variantOrName,
...
---@description Adds multiple DX-style model variants in one call.
---@added sm64dx
local function character_add_model_variants(charNum, variants)
    if type(variants) ~= TYPE_TABLE then return {} end
    local added = {}
    for i = 1, #variants do
        added[i] = character_add_model_variant(charNum, variants[i])
    end
    return added
end

---@description Adds multiple DX-style geo variants in one call.
---@added sm64dx
local function character_add_geo_variants(charNum, variants)
    if type(variants) ~= TYPE_TABLE then return {} end
    local added = {}
    for i = 1, #variants do
        added[i] = character_add_geo_variant(charNum, variants[i])
    end
    return added
end

---@description Convenience helper for editing `_geo.bin`/`_geo` model variants with DX naming.
---@added sm64dx
local function character_edit_geo_variant(charNum, charAlt, variantOrName, description, credit, color, geoBin, baseChar, lifeIcon, camScale)
    if type(variantOrName) == TYPE_TABLE then
        return character_edit_variant(charNum, charAlt, variantOrName)
    end
    return character_edit_variant(charNum, charAlt, {
        name = variantOrName,
        description = description,
        credit = credit,
        color = color,
...
local function character_get_current_variant(localIndex)
    local charNum = character_get_current_number(localIndex)
    local charAlt = character_get_current_costume(localIndex)
    if charNum == nil or charAlt == nil then
        return nil
    end
    return characterTable[charNum] and characterTable[charNum][charAlt] or nil
end
...
---@note This assumes multiple characters will not have the same model, Icons can only be seen by users who have the character avalible to them. This function can return nil. if this is the case, render `mxui_hud_print_text("?", x, y, 1)`
...
local function set_menu_open(bool)
    ...
    if bool then
        if game_menu_open_character_select ~= nil then
            game_menu_open_character_select()
        end
    elseif character_dx_cleanup_runtime ~= nil then
        character_dx_cleanup_runtime()
    end
end
...
local function restrict_palettes(bool)
    if not char_select_has_runtime_authority() then return end
    if bool == nil then bool = true end
...
---@description A function that returns if the character number is of a character that is included with SM64DX
---@added 1.15.1
...
_G.charSelect = {
    ...
    character_add_variant = character_add_variant,
    character_add_model_variant = character_add_model_variant,
    character_add_model_variants = character_add_model_variants,
    character_add_geo_variant = character_add_geo_variant,
    character_add_geo_variants = character_add_geo_variants,
    character_add_costume = character_add_costume,
    character_edit = character_edit,
    character_edit_variant = character_edit_variant,
    character_edit_model_variant = character_edit_model_variant,
    character_edit_geo_variant = character_edit_geo_variant,
    ...
    character_get_current_variant = character_get_current_variant,
    ...
    dx_cleanup_runtime = character_dx_cleanup_runtime,
    dx_save_preferred_number = character_dx_save_preferred_number,
    dx_open_browser = character_dx_open_browser,
    dx_open_browser_tab = character_dx_open_browser_tab,
    dx_close_browser = character_dx_close_browser,
    dx_is_browser_open = character_dx_is_browser_open,
    dx_preview_select = character_dx_preview_select,
    dx_reset_colors = character_dx_reset_colors,

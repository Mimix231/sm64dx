...
-- description:\\#ffff33\\-- Character Select DX v1.16.3 --\n\n\\#dcdcdc\\A character and variant library for SM64DX.\nOpen it from\\#ffff33\\ Player > Open Character Select\\#dcdcdc\\ or bind the hotkey in Mod Controls.\n\nOriginal mod by:\\#008800\\ Squishy6094\n\\#AAAAFF\\Adapted for SM64DX
-- pausable: false
-- category: cs
...
local function create_hook_wrapper(hookEventType)
    ...
    hook_event(hookEventType, function(...)
        for _, func in pairs(callbacks) do
            ...
            if not ok then
                print("[Character Select DX] hook " .. tostring(hookEventType) .. " failed: " .. tostring(err))
            end
        end
    end)
...
local TEXT_TABLE_MENU_BINDS = {
    [MENU_BINDS_DEFAULT] = {
        ...
        {bind = "Left / Right", desc = "Change Variant"},
...
local menuOpenBind = { 0x002E, Z_TRIG, VK_INVALID }
local menuQueuedFromManager = false
local menuQueuedFromManagerFrames = 0
local dxUiOpen = false
local dxUiIgnoreCloseUntilRelease = false
local dxUiStartupFrames = 0
local dxUiLastError = nil
local CHARACTER_SELECT_DX_ONLY = true
local dxUiTab = 1
local dxUiQueuedTab = 1
local dxUiModelPage = 0
local dxUiVariantPage = 0
local dxUiPalettePage = 0
local dxUiPrevChar = bootChar
local dxUiPrevAlt = 1
local dxUiPrevPalette = 0
local open_character_select_dx = nil

local function cs_popup(message, lines)
    mxui_popup_create(message, lines or 2)
end

local function cs_notice(message, lines)
    mxui_popup_create(message, lines or 3)
end

local function dx_is_game_menu_open()
    if type(is_game_menu_open) ~= TYPE_FUNCTION then
        return false
    end
    local ok, result = pcall(is_game_menu_open)
    return ok and result == true
end

local function dx_close_game_menu()
    if type(game_menu_close) ~= TYPE_FUNCTION then
        return false
    end
    local ok, result = pcall(game_menu_close)
    return ok and result == true
end

local function dx_is_frontend_menu_open()
    if type(is_frontend_menu_open) ~= TYPE_FUNCTION then
        return false
    end
    local ok, result = pcall(is_frontend_menu_open)
    return ok and result == true
end
...
characterCategories = {
    ...
    {name = "DX Cast", icon1 = CT_WARIO, icon2 = CT_WALUIGI},
}
...
optionTable = {
    [optionTableRef.openInputs] = {
        ...
        toggleDefault = 0,
        toggleMax = 0,
        toggleNames = {"See Mod Controls"},
        description = {"Change Character Select's hotkey", "from SM64 DX -> Controls ->", "Mod Controls."},
        lock = function ()
            return "Mod Controls"
        end,
    },
    ...
    [optionTableRef.restrictMovesets] = {
        ...
        toggleDefault = 0,
        toggleMax = 1,
        description = {"Restricts turning on movesets", "for this session"},
        lock = function ()
            if gGlobalSyncTable.charSelectRestrictMovesets >= 2 then
                return "Pack Locked"
            end
            if not char_select_has_runtime_authority() then
                return char_select_authority_label()
            end
        end,
    },
...
local function reset_options(wasChatTriggered)
    if not promptedAreYouSure then
        cs_notice("\\#ffdcdc\\Are you sure you want to reset your Save Data for Character Select, including your Preferred Character\nand Settings?\nSelect the \\#ff3333\\" .. optionTable[optionTableRef.resetSaveData].name .. "\\#ffdcdc\\ option again to confirm.", 5)
        ...
        cs_notice("\\#ff3333\\Character Select Save Data Reset!", 2)
        cs_notice("Note: If your issue has not been resolved, you may need to manually delete Character Select DX save data from your SM64DX save folder.", 4)
...
local function boot_note()
    ...
    if charCount > 0 then
        cs_notice("Character Select has " .. charCount .. " character" .. (charCount > 1 and "s" or "") .." available!\nOpen it from \\#ffff33\\Player > Open Character Select\\#ffffff\\ or bind the hotkey in Mod Controls.", 4)
        if charCount > 32 then
            cs_notice("\\#FFAAAA\\Warning: Having a lot of characters\nmay be unstable. For a better experience,\ndisable a few packs.", 4)
        end
    else
        cs_notice("Character Select DX is active!\nOpen it from \\#ffff33\\Player > Open Character Select\\#ffffff\\ or bind the hotkey in Mod Controls.", 3)
    end
end

local function menu_is_allowed(m)
    if m == nil then m = gMarioStates[0] end
    if not CHARACTER_SELECT_DX_ONLY then
        -- API Check
...
local function open_menu_from_mod_manager()
    character_dx_open_browser()
end

hook_mod_menu_button("Open Character Select", open_menu_from_mod_manager)
hook_mod_menu_bind("Open Menu", menuOpenBind, function (_, binds)
    menuOpenBind = binds
end)
...
local function mario_update(m)
    ...
    local currTable = characterTable[currChar]
    if currTable == nil or currTable[currTable.currAlt] == nil then
        currChar = bootChar
        currCharRender = bootChar
        update_character_render_table()
        currTable = characterTable[currChar]
        if currTable == nil or currTable[currTable.currAlt] == nil then
            return
        end
    end
    ...
    if m.playerIndex == 0 then
        if dx_is_game_menu_open() and not dxUiOpen then
            force_close_character_select_menu(m, p)
        end
        ...
        if CHARACTER_SELECT_DX_ONLY then
            if menu or menuAndTransition or p.inMenu then
                character_dx_clear_legacy_runtime(m, p, true)
            end
            menu = false
            menuAndTransition = false
            options = nil
        ...
        if menu and options == OPTIONS_MAIN then
            if char_select_has_runtime_authority() and gGlobalSyncTable.charSelectRestrictMovesets < 2 then
        ...
        if menuAndTransition then
            ...
            local widthScale = mxui_hud_get_screen_width()/320
...
function geo_function()
    ...
    local localPlayer = gCSPlayers[0]
    local menuViewportActive = menuAndTransition and localPlayer ~= nil and localPlayer.inMenu and not dx_is_game_menu_open()

    if dx_is_game_menu_open() or not menuViewportActive then
        viewport.x = 320*0.5
        viewport.y = 240*0.5
        viewport.width = 320*0.5
        viewport.height = 240*0.5
    elseif menuViewportActive then
        mxui_hud_set_resolution(RESOLUTION_N64)
        viewport.x = 320*0.85
        viewport.y = 205*0.5
...
function set_model(o, model, extendedModel, charNum)
    if CHARACTER_SELECT_DX_ONLY then return end
...
local function mario_update_safe(m)
    local ok, result = pcall(function()
        return mario_update(m)
    end)
    ...
    if m ~= nil and m.playerIndex == 0 then
        if dxUiOpen and CHARACTER_SELECT_DX_ONLY then
            dxUiLastError = tostring(result)
            camera_unfreeze()
            hud_show()
            set_override_fov(0)
            local p = gCSPlayers[m.playerIndex]
            ...
            print("[Character Select DX] mario_update failed: " .. tostring(result))
            return 0
        end
        local p = gCSPlayers[m.playerIndex]
        dxUiOpen = false
        ...
        menuAndTransition = false
        options = nil
        menuQueuedFromManager = false
        menuQueuedFromManagerFrames = 0
        if p ~= nil then
            pcall(force_close_character_select_menu, m, p)
        end
    end
...
cs_hook_mario_update(mario_update_safe)
...
local function mxui_hud_render_caution_tape(x1, y1, x2, y2, dist, scale)
    ...
    mxui_hud_set_rotation(angle, 0, 0.5)
    ...
    for i = 0, math.floor(tapeSegments) do
        ...
        mxui_hud_render_texture_tile(TEX_CAUTION_TAPE,
...
local function character_dx_clear_legacy_runtime(m, p, keepBrowser)
    if m == nil then m = gMarioStates[0] end
    ...
    if m ~= nil and m.marioObj ~= nil then
        m.marioObj.header.gfx.sharedChild.hookProcess = 1
    end
    ...
    if m ~= nil and m.area ~= nil and m.area.camera ~= nil and m.area.camera.cutscene == CUTSCENE_CS_MENU then
        m.area.camera.cutscene = CUTSCENE_STOP
    end
...
local function force_close_character_select_menu(m, p)
    character_dx_clear_legacy_runtime(m, p, false)
end

function character_dx_cleanup_runtime()
    force_close_character_select_menu(gMarioStates[0], gCSPlayers[0])
end

function character_dx_save_preferred_number(charNum, charAlt)
    if type(charNum) ~= TYPE_INTEGER or characterTable[charNum] == nil then
        return false
    end

    if charAlt == nil then
        charAlt = characterTable[charNum].currAlt
    end
    charAlt = math.clamp(charAlt, 1, #characterTable[charNum])
    characterTable[charNum].currAlt = charAlt
    mod_storage_save_pref_char(characterTable[charNum])
    return true
end

local function character_dx_current_palette()
    local _, char = dx_ui_get_character_variant(currChar)
    if char == nil then
        return 0
    end
    local model = char.model
    local palettes = characterColorPresets[model]
    return palettes ~= nil and palettes.currPalette or 0
end

local function character_dx_find_first_valid()
    for i = 0, #characterTable do
        local charTable = characterTable[i]
        if charTable ~= nil then
            local alt = tonumber(charTable.currAlt) or 1
            alt = math.floor(alt)
            if alt < 1 or charTable[alt] == nil then
                alt = 1
            end
            if charTable[alt] ~= nil then
                return i, alt
            end
        end
    end
...
local function character_dx_ensure_selection()
    local charTable = characterTable[currChar]
    if charTable ~= nil then
        local alt = tonumber(charTable.currAlt) or 1
        alt = math.floor(alt)
        if alt >= 1 and charTable[alt] ~= nil then
            return currChar, alt
        end
    end

    update_character_render_table()
    charTable = characterTable[currChar]
    if charTable ~= nil then
        local alt = tonumber(charTable.currAlt) or 1
        alt = math.floor(alt)
        if alt >= 1 and charTable[alt] ~= nil then
    ...
    local fallbackChar, fallbackAlt = character_dx_find_first_valid()
    if fallbackChar == nil or fallbackAlt == nil then
        return nil, nil
    end

    force_set_character(fallbackChar, fallbackAlt)
    update_character_render_table()
    return fallbackChar, fallbackAlt
end

local function character_dx_restore_previous_choice()
    force_set_character(dxUiPrevChar, dxUiPrevAlt)
    local _, char = dx_ui_get_character_variant(currChar)
    if char == nil then
        return
    end
    ...
    if palettes ~= nil then
        palettes.currPalette = math.clamp(dxUiPrevPalette or 0, 0, #palettes)
    end
end

function character_dx_preview_select(charNum, charAlt)
    if not character_set_current_number(charNum, charAlt) then
        return false
    end
    update_character_render_table()
    return true
...
function character_dx_open_browser()
    if dx_is_frontend_menu_open() then
        cs_popup("Start an adventure first", 1)
        return false
    end
    if dx_is_game_menu_open() then
        dxUiQueuedTab = 1
        menuQueuedFromManager = true
        menuQueuedFromManagerFrames = 2
        dx_close_game_menu()
        return true
    end
    if not menu_is_allowed(gMarioStates[0]) then
        cs_popup(TEXT_PAUSE_UNAVAILABLE, 2)
        return false
    end

    local resolvedChar, resolvedAlt = character_dx_ensure_selection()
    if resolvedChar == nil or resolvedAlt == nil then
        cs_popup("Character Select is unavailable right now.", 2)
        return false
    end

    force_close_character_select_menu(gMarioStates[0], gCSPlayers[0])
    dxUiPrevChar = resolvedChar
    dxUiPrevAlt = resolvedAlt
    dxUiPrevPalette = character_dx_current_palette()
    dxUiOpen = true
    dxUiIgnoreCloseUntilRelease = true
    dxUiStartupFrames = 0
    dxUiLastError = nil
    dxUiTab = 1
    dxUiQueuedTab = 1
    ...
    hud_show()
    set_override_fov(0)
    force_set_character(resolvedChar, resolvedAlt)
    update_character_render_table()
    return true
end

function character_dx_open_browser_tab(tab)
    local desiredTab = math.clamp(tab or 1, 1, 4)
    if dx_is_frontend_menu_open() then
        cs_popup("Start an adventure first", 1)
        return false
    end
    if dx_is_game_menu_open() then
        dxUiQueuedTab = desiredTab
        menuQueuedFromManager = true
        menuQueuedFromManagerFrames = 2
        dx_close_game_menu()
        return true
    end
    if not character_dx_open_browser() then
        return false
    end
    dxUiTab = desiredTab
    dxUiModelPage = 0
...
function character_dx_reset_colors()
    ...
    if char ~= nil and char[char.currAlt] ~= nil then
        local palettes = characterColorPresets[char[char.currAlt].model]
        if palettes ~= nil then
            palettes.currPalette = 0
        end
    end
    return true
end

function character_dx_close_browser(restorePrevious)
    if restorePrevious then
        character_dx_restore_previous_choice()
    end
    dxUiOpen = false
    dxUiIgnoreCloseUntilRelease = false
    dxUiStartupFrames = 0
    dxUiLastError = nil
    force_close_character_select_menu(gMarioStates[0], gCSPlayers[0])
end

function character_dx_is_browser_open()
    return dxUiOpen
end

open_character_select_dx = function()
    return character_dx_open_browser()
end

--Basic Menu Text
local yearsOfCS = get_date_and_time().year - 123 -- Zero years as of 2023
local TEXT_VERSION = "Version: " .. MOD_VERSION_STRING .. " | sm64dx" .. (seasonalEvent == SEASON_EVENT_BIRTHDAY and (" | " .. tostring(yearsOfCS) .. " year" .. (yearsOfCS > 1 and "s" or "") .. " of Character Select!") or "")
...
local DX_TAB_MODELS = 1
local DX_TAB_VARIANTS = 2
local DX_TAB_COLORS = 3
local DX_TAB_SETTINGS = 4

local function dx_ui_mouse_in_rect(x, y, w, h)
    local mx = mxui_hud_get_mouse_x()
    local my = mxui_hud_get_mouse_y()
    return mx >= x and mx <= x + w and my >= y and my <= y + h
end

local function dx_ui_left_click_pressed()
    return (mxui_hud_get_mouse_buttons_pressed() & 1) ~= 0
end

local function dx_ui_draw_panel(x, y, w, h, fillA)
    mxui_hud_set_color(menuColorHalf.r * 0.55, menuColorHalf.g * 0.55, menuColorHalf.b * 0.65, fillA or 220)
    mxui_hud_render_rect(x, y, w, h)
    mxui_hud_set_color(menuColorTint.r, menuColorTint.g, menuColorTint.b, 255)
    mxui_hud_render_rect(x, y, w, 2)
    mxui_hud_render_rect(x, y + h - 2, w, 2)
    mxui_hud_render_rect(x, y, 2, h)
    mxui_hud_render_rect(x + w - 2, y, 2, h)
end

local function dx_ui_draw_button(label, x, y, w, h, active, danger)
    local hovered = dx_ui_mouse_in_rect(x, y, w, h)
    local base = danger and { r = 180, g = 70, b = 70 } or { r = 120, g = 180, b = 120 }
    if active then
        base = { r = 245, g = 224, b = 160 }
    elseif hovered then
        base = { r = math.min(base.r + 35, 255), g = math.min(base.g + 35, 255), b = math.min(base.b + 35, 255) }
    end
    mxui_hud_set_color(base.r, base.g, base.b, 235)
    mxui_hud_render_rect(x, y, w, h)
    mxui_hud_set_color(255, 234, 140, 255)
    mxui_hud_render_rect(x, y, w, 1)
    mxui_hud_set_color(255, 255, 255, 255)
    label = tostring(label or "")
    local scale = math.min(1.0, (w - 12) / math.max(mxui_hud_measure_text(label), 1))
    scale = math.max(scale * 0.45, 0.32)
    mxui_hud_print_text(label, x + w * 0.5 - mxui_hud_measure_text(label) * scale * 0.5, y + h * 0.5 - 7, scale)
    return hovered and dx_ui_left_click_pressed()
end

local function dx_ui_select_character(charNum, charAlt)
    charAlt = charAlt or 1
    if character_dx_preview_select(charNum, charAlt) then
        currChar = charNum
        currCharRender = charNum
        characterTable[currChar].currAlt = charAlt
        update_character_render_table()
...
local function dx_ui_safe_text(value, fallback)
    ...
    if type(value) == TYPE_TABLE then
        local buffer = ""
        for i = 1, #value do
            buffer = buffer .. tostring(value[i]) .. (i < #value and " " or "")
        end
        return buffer ~= "" and buffer or fallback
    end
    return tostring(value)
end

local function dx_ui_get_character_variant(charNum, charAlt)
    local charTable = characterTable[charNum]
    if charTable == nil then
        return nil, nil, 1
    end

    local alt = tonumber(charAlt) or tonumber(charTable.currAlt) or 1
    alt = math.floor(alt)
    if alt < 1 or charTable[alt] == nil then
        alt = 1
    end
    if charTable[alt] == nil then
        return charTable, nil, alt
    end
    if charTable.currAlt == nil or charTable[charTable.currAlt] == nil then
        charTable.currAlt = alt
    end
    return charTable, charTable[alt], alt
end

local function dx_ui_icon_is_texture(icon)
    return type(icon) == TYPE_TEX_INFO or (type(icon) == TYPE_TABLE and icon.width ~= nil and icon.height ~= nil)
end

local function dx_ui_draw_icon(icon, color, x, y, scale)
    color = color or { r = 255, g = 255, b = 255 }
    if type(icon) == TYPE_STRING then
        mxui_hud_set_font(FONT_RECOLOR_HUD)
        mxui_hud_set_color(color.r, color.g, color.b, 255)
        mxui_hud_print_text(icon, x, y, scale)
        return
    end
    if dx_ui_icon_is_texture(icon) then
        mxui_hud_set_color(255, 255, 255, 255)
        mxui_hud_render_texture(icon, x, y, scale / (icon.width * MATH_DIVIDE_16), scale / (icon.height * MATH_DIVIDE_16))
        return
    end
    mxui_hud_set_font(FONT_RECOLOR_HUD)
    mxui_hud_set_color(color.r, color.g, color.b, 255)
...
local function dx_ui_render_browser()
    mxui_hud_set_resolution(RESOLUTION_N64)

    local sw = mxui_hud_get_screen_width()
    local sh = mxui_hud_get_screen_height()
    local shellX = 8
    local shellY = 10
    local shellW = sw - 16
    local shellH = sh - 20
    local leftX = shellX + 18
    local leftY = shellY + 48
    local leftW = shellW * 0.52
    local rightX = leftX + leftW + 16
    local rightY = leftY
    local rightW = shellX + shellW - rightX - 18
    local bodyH = shellH - 78

    mxui_hud_set_color(0, 0, 0, 155)
    mxui_hud_render_rect(0, 0, sw, sh)
    dx_ui_draw_panel(shellX, shellY, shellW, shellH, 180)
    local resolvedChar, resolvedAlt = character_dx_ensure_selection()
    if resolvedChar == nil or resolvedAlt == nil then
        mxui_hud_set_font(FONT_MENU)
        mxui_hud_set_color(255, 244, 168, 255)
        mxui_hud_print_text("Character Select DX", shellX + 18, shellY + 14, 0.68)
        mxui_hud_set_font(FONT_NORMAL)
        mxui_hud_set_color(255, 255, 255, 220)
        mxui_hud_print_text("No valid character or variant is available.", shellX + 18, shellY + 46, 0.42)
        if dx_ui_draw_button("Close", shellX + 18, shellY + shellH - 34, 82, 24, false, true) then
            character_dx_close_browser(false)
        end
        return
    end

    currChar = resolvedChar
    local currentTable, currentVariant = dx_ui_get_character_variant(resolvedChar, resolvedAlt)
    if currentTable == nil or currentVariant == nil then
        mxui_hud_set_font(FONT_MENU)
        mxui_hud_set_color(255, 244, 168, 255)
        mxui_hud_print_text("Character Select DX", shellX + 18, shellY + 14, 0.68)
        mxui_hud_set_font(FONT_NORMAL)
        mxui_hud_set_color(255, 255, 255, 220)
        mxui_hud_print_text("The current variant could not be resolved.", shellX + 18, shellY + 46, 0.42)
        if dx_ui_draw_button("Close", shellX + 18, shellY + shellH - 34, 82, 24, false, true) then
            character_dx_close_browser(false)
        end
        return
    ...
    mxui_hud_set_font(FONT_MENU)
    mxui_hud_set_color(255, 244, 168, 255)
    local title = "Character Select DX"
    mxui_hud_print_text(title, shellX + shellW * 0.5 - mxui_hud_measure_text(title) * 0.33, shellY + 10, 0.66)
    mxui_hud_set_font(FONT_NORMAL)
    local subtitle = "Model browser, variants, and live preview."
    mxui_hud_set_color(255, 255, 255, 220)
    mxui_hud_print_text(subtitle, shellX + shellW * 0.5 - mxui_hud_measure_text(subtitle) * 0.24, shellY + 30, 0.48)

    dx_ui_draw_panel(leftX, leftY, leftW, bodyH, 214)
    dx_ui_draw_panel(rightX, rightY, rightW, bodyH, 196)

    local tabLabels = { "Model", "Variants", "Colors", "Settings" }
    local tabW = (leftW - 18) / 4
    for i = 1, 4 do
        if dx_ui_draw_button(tabLabels[i], leftX + 6 + (i - 1) * tabW, leftY + 6, tabW - 6, 24, dxUiTab == i, false) then
            dxUiTab = i
        end
    end

    local listX = leftX + 10
    local listY = leftY + 38
    local listW = leftW - 20
    local listH = bodyH - 48
    local cols = 4
    local rows = 3
    local gap = 8
    local cellW = (listW - gap * (cols - 1)) / cols
    local cellH = (listH - 34 - gap * (rows - 1)) / rows
    local scrollY = mxui_hud_get_mouse_scroll_y()

    if dxUiTab == DX_TAB_MODELS then
        local perPage = cols * rows
        local pageCount = math.max(1, math.ceil((#characterTableRender + 1) / perPage))
        if dx_ui_mouse_in_rect(listX, listY, listW, listH) then
            if scrollY > 0 then dxUiModelPage = math.max(0, dxUiModelPage - 1) end
            if scrollY < 0 then dxUiModelPage = math.min(pageCount - 1, dxUiModelPage + 1) end
        end
        local startIndex = dxUiModelPage * perPage
        for i = 0, perPage - 1 do
            local renderIndex = startIndex + i
            if renderIndex > #characterTableRender then break end
            local row = math.floor(i / cols)
            local col = i % cols
            local x = listX + col * (cellW + gap)
            local y = listY + row * (cellH + gap)
            local charTable = characterTableRender[renderIndex]
            local _, char = dx_ui_get_character_variant(charTable.ogNum, charTable.currAlt)
            if char == nil then
                char = { lifeIcon = "?", color = { r = 255, g = 255, b = 255 } }
            end
            local selected = charTable.ogNum == currChar
            dx_ui_draw_panel(x, y, cellW, cellH, selected and 220 or 165)
            if dx_ui_mouse_in_rect(x, y, cellW, cellH) and dx_ui_left_click_pressed() then
                dx_ui_select_character(charTable.ogNum, 1)
                dxUiVariantPage = 0
                dxUiPalettePage = 0
            end
            dx_ui_draw_icon(char.lifeIcon, char.color, x + 8, y + 8, 1.25)
            mxui_hud_set_font(FONT_NORMAL)
            mxui_hud_set_color(255, 255, 255, 255)
            local label = dx_ui_safe_text(charTable.nickname, dx_ui_safe_text(char.name, "Character"))
            local scale = math.max(math.min((cellW - 12) / math.max(mxui_hud_measure_text(label), 1) * 0.48, 0.52), 0.32)
            mxui_hud_print_text(label, x + cellW * 0.5 - mxui_hud_measure_text(label) * scale * 0.5, y + cellH - 18, scale)
        end
        if dx_ui_draw_button("< Prev", listX, leftY + bodyH - 30, 70, 22, false, false) then dxUiModelPage = math.max(0, dxUiModelPage - 1) end
        if dx_ui_draw_button("Next >", leftX + leftW - 80, leftY + bodyH - 30, 70, 22, false, false) then dxUiModelPage = math.min(pageCount - 1, dxUiModelPage + 1) end
    elseif dxUiTab == DX_TAB_VARIANTS then
        local variants = characterTable[currChar]
        local variantCount = #variants
        local vCols = 2
        local vRows = 5
        local vPerPage = vCols * vRows
        local vCellW = (listW - gap) / vCols
        local vCellH = (listH - 34 - gap * (vRows - 1)) / vRows
        local pageCount = math.max(1, math.ceil(variantCount / vPerPage))
        if dx_ui_mouse_in_rect(listX, listY, listW, listH) then
            if scrollY > 0 then dxUiVariantPage = math.max(0, dxUiVariantPage - 1) end
            if scrollY < 0 then dxUiVariantPage = math.min(pageCount - 1, dxUiVariantPage + 1) end
        end
        local startIndex = dxUiVariantPage * vPerPage + 1
        for i = 0, vPerPage - 1 do
            local variantIndex = startIndex + i
            if variantIndex > variantCount then break end
            local row = math.floor(i / vCols)
            local col = i % vCols
            local x = listX + col * (vCellW + gap)
            local y = listY + row * (vCellH + gap)
            local variant = variants[variantIndex]
            if dx_ui_draw_button(variant.name, x, y, vCellW, vCellH, variantIndex == variants.currAlt, false) then
                dx_ui_select_character(currChar, variantIndex)
            end
        end
        if dx_ui_draw_button("< Prev", listX, leftY + bodyH - 30, 70, 22, false, false) then dxUiVariantPage = math.max(0, dxUiVariantPage - 1) end
        if dx_ui_draw_button("Next >", leftX + leftW - 80, leftY + bodyH - 30, 70, 22, false, false) then dxUiVariantPage = math.min(pageCount - 1, dxUiVariantPage + 1) end
    elseif dxUiTab == DX_TAB_COLORS then
        local palettes = characterColorPresets[currentVariant.model]
        if palettes == nil or #palettes <= 0 then
            mxui_hud_set_color(255, 255, 255, 255)
            local msg = "No palette presets are available for this character."
            mxui_hud_print_text(msg, listX + 8, listY + 16, 0.54)
        else
            local pCols = 4
            local pRows = 3
            local pPerPage = pCols * pRows
            local pageCount = math.max(1, math.ceil((#palettes + 1) / pPerPage))
            if dx_ui_mouse_in_rect(listX, listY, listW, listH) then
                if scrollY > 0 then dxUiPalettePage = math.max(0, dxUiPalettePage - 1) end
                if scrollY < 0 then dxUiPalettePage = math.min(pageCount - 1, dxUiPalettePage + 1) end
            end
            local startIndex = dxUiPalettePage * pPerPage
            for i = 0, pPerPage - 1 do
                local paletteIndex = startIndex + i
                if paletteIndex > #palettes then break end
                local row = math.floor(i / pCols)
                local col = i % pCols
                local x = listX + col * (cellW + gap)
                local y = listY + row * (cellH + gap)
                local shirt = paletteIndex == 0 and network_player_get_palette_color(gNetworkPlayers[0], SHIRT) or palettes[paletteIndex][SHIRT]
                local pants = paletteIndex == 0 and network_player_get_palette_color(gNetworkPlayers[0], PANTS) or palettes[paletteIndex][PANTS]
                dx_ui_draw_panel(x, y, cellW, cellH, palettes.currPalette == paletteIndex and 220 or 165)
                mxui_hud_set_color(shirt.r, shirt.g, shirt.b, 255)
                mxui_hud_render_rect(x + 8, y + 8, cellW - 16, (cellH - 24) * 0.5)
                mxui_hud_set_color(pants.r, pants.g, pants.b, 255)
                mxui_hud_render_rect(x + 8, y + 8 + (cellH - 24) * 0.5, cellW - 16, (cellH - 24) * 0.5)
                local pname = paletteIndex == 0 and "Custom" or (palettes[paletteIndex].name or ("Palette " .. tostring(paletteIndex)))
                mxui_hud_set_color(255, 255, 255, 255)
                local scale = math.max(math.min((cellW - 12) / math.max(mxui_hud_measure_text(pname), 1) * 0.48, 0.44), 0.28)
                mxui_hud_print_text(pname, x + cellW * 0.5 - mxui_hud_measure_text(pname) * scale * 0.5, y + cellH - 14, scale)
                if dx_ui_mouse_in_rect(x, y, cellW, cellH) and dx_ui_left_click_pressed() then
                    palettes.currPalette = paletteIndex
                end
            end
            if dx_ui_draw_button("< Prev", listX, leftY + bodyH - 30, 70, 22, false, false) then dxUiPalettePage = math.max(0, dxUiPalettePage - 1) end
            if dx_ui_draw_button("Next >", leftX + leftW - 80, leftY + bodyH - 30, 70, 22, false, false) then dxUiPalettePage = math.min(pageCount - 1, dxUiPalettePage + 1) end
        end
    else
        local settingsText = "Use C or your Character Select hotkey to open this browser.\nClick models and variants to update the live preview immediately.\nUse Apply & Save to keep the current variant for future sessions."
        mxui_hud_set_color(255, 255, 255, 255)
        mxui_hud_print_text(settingsText, listX + 8, listY + 12, 0.48)
    end

    local previewChar = currentVariant
    local previewName = dx_ui_safe_text(previewChar.name, "Character")
    local previewCategory = dx_ui_safe_text(currentTable.category, "Misc")
    local previewCredit = dx_ui_safe_text(previewChar.credit, "Unknown")
    local previewDescription = dx_ui_safe_text(previewChar.description, "No description provided.")
    local detailY = rightY + 12
    mxui_hud_set_font(FONT_MENU)
    mxui_hud_set_color(255, 244, 168, 255)
    local header = previewName
    mxui_hud_print_text(header, rightX + 16, detailY, 0.52)
    mxui_hud_set_font(FONT_NORMAL)
    mxui_hud_set_color(255, 255, 255, 220)
    mxui_hud_print_text("Category: " .. previewCategory, rightX + 16, detailY + 28, 0.48)
    mxui_hud_print_text("Credit: " .. previewCredit, rightX + 16, detailY + 48, 0.48)
    dx_ui_draw_panel(rightX + 14, rightY + 74, rightW - 28, 170, 132)
    mxui_hud_set_color(255, 255, 255, 210)
    mxui_hud_print_text("Live Preview", rightX + 24, rightY + 88, 0.54)
    mxui_hud_print_text("The player model on the right updates as soon as you click a character or variant.", rightX + 24, rightY + 112, 0.42)

    dx_ui_draw_panel(rightX + 14, rightY + 258, rightW - 28, 120, 148)
    mxui_hud_set_color(255, 255, 255, 255)
    mxui_hud_print_text(previewDescription, rightX + 24, rightY + 272, 0.44)

    local buttonY = rightY + bodyH - 40
    if dx_ui_draw_button("Apply & Save", rightX + 14, buttonY, (rightW - 40) * 0.5, 28, false, false) then
        if character_dx_save_preferred_number(currChar, currentTable.currAlt) then
            character_dx_close_browser(false)
            cs_popup("Character Select saved.", 1)
        else
            cs_popup("Character Select could not save that variant.", 2)
        end
    end
    if dx_ui_draw_button("Use Session", rightX + 22 + (rightW - 40) * 0.5, buttonY, (rightW - 40) * 0.5, 28, false, false) then
        character_dx_close_browser(false)
    end
    if dx_ui_draw_button("Cancel", rightX + 14, buttonY + 34, rightW - 28, 26, false, true) then
        character_dx_close_browser(true)
    end
end

local function dx_ui_render_error_panel(message)
    mxui_hud_set_resolution(RESOLUTION_N64)
    local sw = mxui_hud_get_screen_width()
    local sh = mxui_hud_get_screen_height()
    local shellX = 18
    ...
    mxui_hud_set_font(FONT_MENU)
    mxui_hud_set_color(255, 244, 168, 255)
    mxui_hud_print_text("Character Select DX", shellX + 18, shellY + 16, 0.68)
    mxui_hud_set_font(FONT_NORMAL)
    mxui_hud_set_color(255, 255, 255, 220)
    mxui_hud_print_text("The browser hit an internal error.", shellX + 18, shellY + 48, 0.56)
    mxui_hud_print_text(dx_ui_safe_text(message, "Unknown error."), shellX + 18, shellY + 74, 0.48)
    mxui_hud_print_text("Press B, Start, or C to close.", shellX + 18, shellY + shellH - 28, 0.42)
end

local function mxui_hud_render_life_icon(char, x, y, scale)
    local icon = char and char.lifeIcon or "?"
    local color = char and char.color or {r = 255, g = 255, b = 255}
    local djuiColor = mxui_hud_get_color()
    if type(icon) == TYPE_STRING then
        local font = mxui_hud_get_font()
        mxui_hud_set_font(FONT_RECOLOR_HUD)
        mxui_hud_set_color(color.r * djuiColor.r/255, color.g * djuiColor.g/255, color.b * djuiColor.b/255, djuiColor.a)
        mxui_hud_print_text(icon, x, y, scale)
        mxui_hud_set_font(font)
    else
        mxui_hud_set_color(djuiColor.r, djuiColor.g, djuiColor.b, djuiColor.a)
        mxui_hud_render_texture(icon, x, y, scale / (icon.width * MATH_DIVIDE_16), scale / (icon.height * MATH_DIVIDE_16))
    end
    mxui_hud_set_color(djuiColor.r, djuiColor.g, djuiColor.b, djuiColor.a)
end
...
local function on_hud_render()
    ...
    mxui_hud_set_font(FONT_ALIASED)
    mxui_hud_set_resolution(RESOLUTION_DJUI)
    local djuiWidth = mxui_hud_get_screen_width()
    local djuiHeight = mxui_hud_get_screen_height()
    mxui_hud_set_resolution(RESOLUTION_N64)
    ...
    if dxUiOpen then
        ...
        dxUiStartupFrames = dxUiStartupFrames + 1
        mxui_hud_set_resolution(RESOLUTION_N64)
        if dxUiLastError ~= nil then
            dx_ui_render_error_panel(dxUiLastError)
        else
            local ok, err = pcall(dx_ui_render_browser)
            if ok then
                return
            end
            dxUiLastError = tostring(err)
            dx_ui_render_error_panel(dxUiLastError)
        end
        return
    end
    ...
    if CHARACTER_SELECT_DX_ONLY then
        local localPlayer = gCSPlayers[0]
        if menu or menuAndTransition or (localPlayer ~= nil and localPlayer.inMenu) then
            character_dx_clear_legacy_runtime(gMarioStates[0], localPlayer, true)
        end
        return
    end
    ...
    if menuAndTransition then
        if characterTable[currChar][characterTable[currChar].currAlt].model == E_MODEL_ERROR_MODEL then
            mxui_hud_set_color(0, 0, 0, 200)
            mxui_hud_render_rect(0, 0, width, height)
            mxui_hud_set_color(255, 255, 255, 255)
            mxui_hud_print_text(TEXT_LOCAL_MODEL_ERROR, width*0.85 - mxui_hud_measure_text(TEXT_LOCAL_MODEL_ERROR) * 0.15 * widthScale, height * 0.5, 0.3 * widthScale)
            mxui_hud_print_text(TEXT_LOCAL_MODEL_ERROR_FIX, width*0.85 - mxui_hud_measure_text(TEXT_LOCAL_MODEL_ERROR_FIX) * 0.1 * widthScale, height * 0.5 + 10 * widthScale, 0.2 * widthScale)
        end
        ...
        if width < 319 or width > 575 then
            mxui_hud_print_text(TEXT_RATIO_UNSUPPORTED, 5, 39, 0.5)
        end

        mxui_hud_set_resolution(RESOLUTION_N64)

        mxui_hud_set_color(menuColor.r, menuColor.g, menuColor.b, 255)
        mxui_hud_render_rect(width * 0.5 - 50 * widthScale, height - 2, 100 * widthScale, 2)
        ...
        mxui_hud_set_color(menuColor.r*0.1, menuColor.g*0.1, menuColor.b*0.1, 200)
        mxui_hud_set_rotation(angle1, 0, 1)
        mxui_hud_render_rect(width*0.7, -50, width*0.4, 59)
        mxui_hud_set_rotation(angle2, 0, 1)
        mxui_hud_render_rect(width*0.7, -50, width*0.4, 96)
        mxui_hud_set_color(menuColor.r, menuColor.g, menuColor.b, 255)
        mxui_hud_set_rotation(angle1, 0, 1)
        mxui_hud_render_caution_tape(width*0.7, 8, width*1.1, 30, 1)
        mxui_hud_set_rotation(angle2, 0, 1)
        mxui_hud_render_caution_tape(width*0.7, 40, width*1.1, 35, 1)

        mxui_hud_set_font(FONT_CHARACTERISTIC)
        local charName = string.upper(characterTable[currChar][characterTable[currChar].currAlt].name)
        local nameScale = math.min(width*0.23/mxui_hud_measure_text(charName), 1)
        local charCreator = string.upper(characterTable[currChar][characterTable[currChar].currAlt].credit)
        local creatorScale = math.min(width*0.23/mxui_hud_measure_text(charCreator), 0.4)
        mxui_hud_set_color(menuColorHalf.r, menuColorHalf.g, menuColorHalf.b, 255)
        mxui_hud_print_text_auto_interpolated("topName", charName, width*0.85 - mxui_hud_measure_text(charName)*0.5*nameScale - 2 + menuOffsetX*0.3, 21 - 16*nameScale + menuOffsetY*0.3, nameScale)
        mxui_hud_print_text_auto_interpolated("topCreator", charCreator, width*0.85 - mxui_hud_measure_text(charCreator)*0.5*creatorScale - 2 + menuOffsetX*0.2, 42 - 16*creatorScale + menuOffsetY*0.2, creatorScale)
        ...
        if palettes then
            ...
            for i = 0, #palettes do
                ...
                if paletteShirt and palettePants and (x > width*0.7 - 32 and x < width + 32) then
                    ...
                    mxui_hud_set_color(charColor.r*0.5 + 127, charColor.g*0.5 + 127, charColor.b*0.5 + 127, math.min(paletteTrans, 255))
                    mxui_hud_render_texture_tile(TEX_PALETTE_BUCKET, x, y, 1, 1, 0, bucketFrame, 32, 32)
                    mxui_hud_set_color(paletteShirt.r, paletteShirt.g, paletteShirt.b, math.min(paletteTrans, 255))
                    mxui_hud_render_texture_tile(TEX_PALETTE_BUCKET, x, y, 1, 1, 32, bucketFrame, 32, 32)
                    mxui_hud_set_color(palettePants.r, palettePants.g, palettePants.b, math.min(paletteTrans, 255))
                    mxui_hud_render_texture_tile(TEX_PALETTE_BUCKET, x, y, 1, 1, 64, bucketFrame, 32, 32)

                    if i == palettes.currPalette then
                        mxui_hud_set_color(paletteShirt.r, paletteShirt.g, paletteShirt.b, math.min(paletteTrans, 255))
                        mxui_hud_render_texture_tile(TEX_PALETTE_BUCKET, x, y, 1, 1, 96, bucketPaintFrame, 32, 32)
                        mxui_hud_set_color(palettePants.r, palettePants.g, palettePants.b, math.min(paletteTrans, 255))
                        mxui_hud_render_texture_tile(TEX_PALETTE_BUCKET, x, y, 1, 1, 128, bucketPaintFrame, 32, 32)
                    end
                end
            end
            ...
            mxui_hud_set_font(FONT_RECOLOR_HUD)
            local x = width*0.85 - mxui_hud_measure_text(paletteName)*0.25
            local y = height*0.68 - math.abs(math.cos((get_global_timer() - palettes.currPalette*10)*0.05))*3
            mxui_hud_set_color(charColor.r*0.5 + 127, charColor.g*0.5 + 127, charColor.b*0.5 + 127, math.min(paletteTrans, 255))
            mxui_hud_print_text(paletteName, x, y, 0.5)
        end
        ...
        local scissorWidth = math.max(320/mxui_hud_get_screen_width(), 1)*320*0.7 -- Ensure Wall Space doesn't break when under 4:3
        mxui_hud_set_scissor(0, 0, scissorWidth, height)
        mxui_hud_set_color(playerShirt.r, playerShirt.g, playerShirt.b, 255)
        mxui_hud_render_texture_auto_interpolated("wall-l", TEX_WALL_LEFT, x, y, wallScale, wallScale)
        mxui_hud_set_color(playerPants.r, playerPants.g, playerPants.b, 255)
        mxui_hud_render_texture_auto_interpolated("wall-r", TEX_WALL_RIGHT, x, y, wallScale, wallScale)
        ...
        mxui_hud_set_color(255, 255, 255, 150)
        mxui_hud_render_texture_auto_interpolated("graffiti", graffiti, wallMiddle - graffiti.width*0.5*graffitiWidthScale - menuOffsetX, height*0.5 - graffiti.height*0.5*graffitiHeightScale - menuOffsetY, graffitiWidthScale, graffitiHeightScale)
        ...
        if not gridMenu then
            ...
            mxui_hud_set_font(FONT_SPECIAL)
            for i = 0, #characterTableRender do
                ...
                if i >= currCharScoll - 3 and i <= currCharScoll + 3 then -- Only render if visible
                    ...
                    local charNameLength = mxui_hud_measure_text(charName)
                    ...
                    -- Backlight
                    mxui_hud_set_color(charColor.r*0.5 + 127, charColor.g*0.5 + 127, charColor.b*0.5 + 127, 255)
                    mxui_hud_render_rect(x + 96*scale, y + 24*scale, (128*scale + segments*16*scale), 80*scale)
                    -- Name Screen
                    mxui_hud_set_color(charColor.r*0.5, charColor.g*0.5, charColor.b*0.5, 255)
                    mxui_hud_print_text(charName, x + 112*scale + segments*16*scale*0.5 - charNameLength*textScale*0.5, y + 32*scale, textScale)
                    
                    -- Bottom Info
                    mxui_hud_render_rect(x + 112*scale, y + 84*scale, segments*16*scale, scale)
                    mxui_hud_print_text(channel, x + 112*scale, y + 85*scale, 0.3*scale)
                    mxui_hud_print_text(charAltName, x + segments*16*scale + 112*scale - mxui_hud_measure_text(charAltName)*0.3*scale, y + 85*scale, 0.3*scale)

                    -- Icon
                    mxui_hud_set_color(charColor.r, charColor.g, charColor.b, 150)
                    mxui_hud_render_life_icon(char, x + 112*scale + segments*16*scale + 45*scale, y + 40*scale, scale*3)
                    -- Nameplate Rendering
                    mxui_hud_set_color(menuColorTint.r, menuColorTint.g, menuColorTint.b, 255)
                    mxui_hud_render_texture_tile(TEX_NAMEPLATE, 0, y, (128/8)*x*0.5*scale, scale, 0, 0, 8, 128) -- stretch to left side of screen
                    mxui_hud_render_texture_tile(TEX_NAMEPLATE, x, y, (128/112)*scale, scale, 0, 0, 112, 128)
                    for s = 1, segments do
                        mxui_hud_render_texture_tile(TEX_NAMEPLATE, x + (112 + (s-1)*16)*scale, y, scale*8, scale, 112, 0, 16, 128)
                    end
                    mxui_hud_render_texture_tile(TEX_NAMEPLATE, x + (112 + segments*16)*scale, y, scale*128/192, scale, 128, 0, 192, 128)
                    local angle = -0x10000*((characterTableRender[i].currAlt - 1)/charAltCount)
                    if i == currCharRender then
                        mxui_hud_render_texture_tile(TEX_NAMEPLATE, x + 33*scale, y + 48*scale, scale, scale, 320, 48, 32, 32)
                    end
                    ...
                    mxui_hud_set_rotation(angle + angleAnim, 0.5, 0.5)
                    mxui_hud_render_texture_tile(TEX_NAMEPLATE, x + (112 + segments*16 + 134)*scale, y + 48*scale, scale, scale, 352, 48, 32, 32)
                    mxui_hud_set_rotation(0, 0, 0)
                    for a = 1, charAltCount do
                        local angle = -0x10000*((a - 1)/charAltCount) + 0x8000
                        local altColor = characterTableRender[i][a].color
                        mxui_hud_set_color(altColor.r * 0.5 + 127, altColor.g * 0.5 + 127, altColor.b * 0.5 + 127, 255)
                        mxui_hud_render_texture_tile(TEX_NAMEPLATE, x + (112 + segments*16 + (134 + 14) + sins(angle)*16)*scale, y + (62 + coss(angle)*16)*scale, scale, scale, 384, 48 + (currAlt ~= a and 16 or 0), 4, 4)
                    end
                end
            end
        else
            ...
            for i = 0, #characterTableRender do
                ...
                mxui_hud_set_color(charColor.r, charColor.g, charColor.b, 255)
                if characterInstrumentals[i] ~= nil then
                    mxui_hud_render_texture(TEX_ALBUM_LAYER1, x + 3, y, 0.1875, 0.1875)
                    ...
                    if (i == currCharRender) then
                        discX = x + 13
                        discY = y + 2
                        mxui_hud_set_rotation(math.s16(get_global_timer()*-0x1000), 0.5, 0.5)
                        x = x - 4
                    end
                    mxui_hud_set_color(discColors[1].r, discColors[1].g, discColors[1].b, 255)
                    mxui_hud_render_texture(TEX_CD_LAYER1, discX, discY, 0.15625, 0.15625)
                    mxui_hud_set_color(discColors[2].r, discColors[2].g, discColors[2].b, 255)
                    mxui_hud_render_texture(TEX_CD_LAYER2, discX, discY, 0.15625, 0.15625)
                    mxui_hud_set_color(discColors[3].r, discColors[3].g, discColors[3].b, 255)
                    mxui_hud_render_texture(TEX_CD_LAYER3, discX, discY, 0.15625, 0.15625)
                    mxui_hud_set_color(255, 255, 255, 255)
                    mxui_hud_render_texture(TEX_CD_LAYER4, discX, discY, 0.15625, 0.15625)
                    mxui_hud_set_rotation(0, 0.5, 0.5)
                elseif i == currCharRender then
                    mxui_hud_render_texture(TEX_ALBUM_LAYER1, x + 6, y, 0.1875, 0.1875)
                    x = x - 4
                else
                    mxui_hud_render_texture(TEX_ALBUM_LAYER1, x + 3, y, 0.1875, 0.1875)
                end
                mxui_hud_set_color(charColor.r, charColor.g, charColor.b, 255)
                mxui_hud_render_texture(TEX_ALBUM_LAYER2, x, y, 0.1875, 0.1875)
                x = x + 4
                y = y + 4
                mxui_hud_set_color(255, 255, 255, 255)
                if type(charIcon) == TYPE_STRING then
                    mxui_hud_set_font(FONT_RECOLOR_HUD)
                    mxui_hud_set_color(charColor.r, charColor.g, charColor.b, 255)
                    mxui_hud_print_text(charIcon, x, y, 1)
                else
                    mxui_hud_render_texture(charIcon, x, y, 1 / (charIcon.width * MATH_DIVIDE_16), 1 / (charIcon.height * MATH_DIVIDE_16))
                end
                mxui_hud_render_texture(TEX_ALBUM_LAYER3, x - 4, y - 4, 0.1875, 0.1875)
        ...
        -- Render OST Record
        mxui_hud_set_color(255, 255, 255, 255)
        mxui_hud_set_rotation(get_global_timer() * 0x10, 0.5, 0.5)
        mxui_hud_render_texture(TEX_RECORD, -152 - menuOffsetX*0.1 - optionsMenuOffset, height*0.5 - 96 - menuOffsetY*0.1, 0.75, 0.75)
        mxui_hud_set_rotation(0, 0, 0)

        -- Render Category Gear
        mxui_hud_set_color(menuColorTint.r, menuColorTint.g, menuColorTint.b, 255)
        mxui_hud_set_rotation(gearRotation, 0.5, 0.5)
        gearRotation = math.lerp(gearRotation, gearRotationTarget, 0.1)
        mxui_hud_render_texture(TEX_GEAR, width*0.7 - 15 - TEX_GEAR.width*0.175 - menuOffsetX*0.1, -TEX_GEAR.height*0.175 - menuOffsetY*0.1, 0.35, 0.35)
        mxui_hud_set_rotation(0, 0, 0)
        ...
        mxui_hud_render_life_icon(char1, width*0.7 - 30 - 4 - menuOffsetX*0.1, 10 - 4 - menuOffsetY*0.1, 1)
        mxui_hud_render_life_icon(char2, width*0.7 - 30 + 4 - menuOffsetX*0.1, 10 + 4 - menuOffsetY*0.1, 1)
        mxui_hud_set_font(FONT_NORMAL)
        mxui_hud_print_text(name, width*0.7 - 65 - mxui_hud_measure_text(name)*0.4 - menuOffsetX*0.1, 2 - menuOffsetY*0.1, 0.4)
        mxui_hud_set_color(255, 255, 255, 255)
        ...
        mxui_hud_set_color(255, 255, 255, 255)
        mxui_hud_render_rect(tvX, tvY, tvWidth, tvHeight)
        if options == OPTIONS_MAIN then
            mxui_hud_set_font(FONT_TINY)
            mxui_hud_set_color(0, 0, 0, 255)
            mxui_hud_print_text(optionData.name, tvX + 12 + (tvWidth - 12)*0.5 - mxui_hud_measure_text(optionData.name)*0.35, tvY + 20, 0.7)
            ...
            mxui_hud_print_text(toggleString, tvX + 12 + (tvWidth - 12)*0.5 - mxui_hud_measure_text(toggleString)*0.25, tvY + 30, 0.5)

            for i = 1, #optionData.description do
                local textMeasure = mxui_hud_measure_text(optionData.description[i])
                local x = tvX + 12 + (tvWidth - 12)*0.5 - textMeasure*0.225
                local y = tvY + tvHeight - 7*(#optionData.description + 2) + 7*i
                mxui_hud_set_color(0, 0, 0, 255)
                mxui_hud_render_rect(x - 2, y, textMeasure*0.45 + 4, 8)
                mxui_hud_set_color(255, 255, 255, 255)
                mxui_hud_print_text(optionData.description[i], x, y, 0.45)
            end

            -- Render Header
            mxui_hud_set_color(0, 0, 0, 255)
            mxui_hud_render_rect(tvX, tvY, tvWidth, 18)
            mxui_hud_set_font(FONT_ALIASED)
            mxui_hud_set_color(255, 255, 255, 255)
            mxui_hud_print_text("OPTIONS", tvX + 13, tvY + 2, 0.5)
            mxui_hud_set_font(FONT_NORMAL)
            local optionCategory = "... " .. string.upper(optionData.category)
            mxui_hud_print_text(optionCategory, tvX + 13 + mxui_hud_measure_text("OPTIONS")*0.5, tvY + 8, 0.25)

            -- Render Sidebar
            mxui_hud_set_color(0, 0, 0, 255)
            mxui_hud_render_rect(tvX, tvY, 10, tvHeight)
            for i = 1, #optionTable do
                ...
                mxui_hud_render_rect(tvX + 4, tvY + (tvHeight - 10)*(i/#optionTable), 2, 2)
            end
        elseif options == OPTIONS_CREDITS then
            creditScrollMin = (currCredits == 0 and 6 or 8)
            -- Render Scroll Bar
            mxui_hud_set_color(0, 0, 0, 255)
            mxui_hud_render_rect(tvX + 3, tvY + 24, 1, 40)
            ...
            mxui_hud_render_rect(tvX + 2, tvY + 24 + 40*(1-creditSize)/math.max(creditLength - creditScrollMin, 1)*(currCreditScroll), 3, 40*creditSize)
            -- Render Credits
            mxui_hud_set_color(0, 0, 0, 255)
            mxui_hud_set_font(FONT_TINY)
            if currCredits > 0 then
                for i = 1, #creditTable[currCredits] do
                    ...
                    mxui_hud_print_text(creditData.creditee, tvX + tvWidth*0.5 - 1 - mxui_hud_measure_text(creditData.creditee)*0.4, tvY + 19 + 6*(i - currCreditScroll), 0.4)
                    mxui_hud_print_text(creditData.credit, tvX + tvWidth*0.5 + 1, tvY + 19 + 6*(i - currCreditScroll), 0.4)
                end
            else
                for i = 1, #creditTable[currCredits] do
                    local creditData = creditTable[currCredits][i]
                    mxui_hud_print_text(creditData.creditee, tvX - tvWidth*0.1 + tvWidth*0.3*(((i - 1)%3) + 1) - mxui_hud_measure_text(creditData.creditee)*0.2, tvY + 19 + 6*(math.ceil(i/3) - currCreditScroll), 0.4)
                end

                -- Render Support Link
                mxui_hud_set_color(0, 0, 0, 255)
                mxui_hud_render_rect(tvX, tvY + tvHeight - 10, tvWidth, 11)
                mxui_hud_set_color(255, 255, 255, 255)
                mxui_hud_set_font(FONT_TINY)
                mxui_hud_print_text(TEXT_KOFI_LINK, tvX + tvWidth*0.5 - mxui_hud_measure_text(TEXT_KOFI_LINK)*0.25, tvY + tvHeight - 10, 0.5)
            end
            -- Render Pack Name
            mxui_hud_set_color(0, 0, 0, 255)
            mxui_hud_render_rect(tvX, tvY, tvWidth, 21)
            mxui_hud_set_color(255, 255, 255, 255)
            mxui_hud_set_font(FONT_ALIASED)
            mxui_hud_print_text(TEXT_CREDITS_HEADER, tvX + tvWidth*0.5 - mxui_hud_measure_text(TEXT_CREDITS_HEADER)*0.2, tvY + 2, 0.4)
            mxui_hud_set_font(FONT_SPECIAL)
            mxui_hud_print_text(creditTable[currCredits].packName, tvX + tvWidth*0.5 - mxui_hud_measure_text(creditTable[currCredits].packName)*0.1, tvY + 14, 0.2)
        else
            local tvShutOffHeight = tvHeight*0.5*(optionsMenuOffsetMax^3.5 - optionsMenuOffset^3.5)/optionsMenuOffsetMax^3.5
            mxui_hud_set_color(0, 0, 0, 255)
            mxui_hud_render_rect(tvX, tvY, tvWidth, tvShutOffHeight)
            mxui_hud_render_rect(tvX, tvY + tvHeight - tvShutOffHeight, tvWidth, tvShutOffHeight)
        end

        mxui_hud_set_color(menuColorTint.r, menuColorTint.g, menuColorTint.b, 255)
        mxui_hud_render_texture_auto_interpolated("tvBoarder", TEX_OPTIONS_TV, tvX - 133*tvScale, tvY - 168*tvScale, tvScale, tvScale)

        mxui_hud_reset_scissor()

        -- Render Character Description
        mxui_hud_set_rotation(angle_from_2d_points(-10, height - 50, width + 10, height - 35), 0, 0)
        mxui_hud_set_color(0, 0, 0, 255)
        mxui_hud_render_rect(-10, height - 50, width*1.5, 100)
        mxui_hud_set_rotation(0, 0, 0)
        ...
        while mxui_hud_measure_text(descRender)*0.8 < width do
            descRender = descRender .. " - " .. desc
        end
        descRender = descRender .. " - " .. desc
        mxui_hud_print_text_interpolated(descRender, 5 - (get_global_timer()%mxui_hud_measure_text(desc .. " - ") - 1)*0.8 - menuOffsetX*0.1, height - 25 + menuOffsetY*0.15, 0.8, 5 - get_global_timer()%mxui_hud_measure_text(desc .. " - ")*0.8 - menuOffsetX*0.1, height - 25 + menuOffsetY*0.15, 0.8)

        mxui_hud_set_rotation(angle_from_2d_points(-10, height - 50, width + 10, height - 35), 0, 0)
        mxui_hud_set_color(0, 0, 0, 255)
        mxui_hud_render_rect(0, height - 45, width*0.3, 100)
        mxui_hud_set_rotation(0, 0, 0)

        mxui_hud_set_color(menuColor.r, menuColor.g, menuColor.b, 255)
        mxui_hud_print_text(TEXT_VERSION, 2, height - 7, 0.4)
        ...
        mxui_hud_print_text(bindInfo.bind, width*0.15 - mxui_hud_measure_text(bindInfo.bind)*0.4, height - 35, 0.8)
        mxui_hud_print_text(bindInfo.desc, width*0.15 - mxui_hud_measure_text(bindInfo.desc)*0.4, height - 25, 0.8)
        ...
        -- Render Header BG
        mxui_hud_set_rotation(angle_from_2d_points(-10, 50, 160, -10), 0.5, 0.5)
        mxui_hud_set_color(0, 0, 0, 255)
        mxui_hud_render_rect(-150, -50, 300, 100)

        -- Render Tape
        mxui_hud_set_color(menuColor.r, menuColor.g, menuColor.b, 255)
        mxui_hud_render_caution_tape(-10, 50, 160, -10, 1) -- Top Tape
        mxui_hud_render_caution_tape(width*0.7 - 5, -10, width*0.7 + 5, height - 35, 1, 0.6) -- Right Tape
        mxui_hud_render_caution_tape(width*0.3, height - 45, width*0.3 - 5, height, 1, 0.5) -- Left Bottom Tape
        mxui_hud_render_caution_tape(-10, height - 50, width + 10, height - 35, 1) -- Bottom Tape

        -- Render Header
        mxui_hud_set_rotation(0, 0, 0)
        mxui_hud_set_color(menuColorHalf.r, menuColorHalf.g, menuColorHalf.b, 255)
        mxui_hud_render_texture_auto_interpolated("logo", TEX_LOGO, menuOffsetX*0.1, menuOffsetY*0.1, 0.25, 0.25)
    ...
    -- Pause menu character info
    if is_game_paused() and not dx_is_game_menu_open() and gMarioStates[0].action ~= ACT_EXIT_LAND_SAVE_DIALOG then
        local currCharY = 0
        mxui_hud_set_resolution(RESOLUTION_DJUI)
        mxui_hud_set_font(FONT_USER)
        ...
        width = mxui_hud_get_screen_width() - mxui_hud_measure_text(TEXT_PAUSE_CURR_CHAR_WITH_NAME)
        local charColor = character.color
        mxui_hud_set_color(255, 255, 255, 255)
        mxui_hud_print_text(TEXT_PAUSE_CURR_CHAR, width - 20, 16, 1)
        mxui_hud_set_color(charColor.r, charColor.g, charColor.b, 255)
        mxui_hud_print_text(charName, mxui_hud_get_screen_width() - mxui_hud_measure_text(charName) - 20, 16, 1)
        ...
        if text ~= nil then
            width = mxui_hud_get_screen_width() - mxui_hud_measure_text(text)
            mxui_hud_set_color(255, 255, 255, 255)
            currCharY = currCharY + 27
            mxui_hud_print_text(text, width - 20, 16 + currCharY, 1)
        end
    elseif is_game_paused() and not dx_is_game_menu_open() and gMarioStates[0].action ~= ACT_EXIT_LAND_SAVE_DIALOG and not menu_is_allowed() then
        width = mxui_hud_get_screen_width() - mxui_hud_measure_text(TEXT_PAUSE_UNAVAILABLE)
        mxui_hud_set_resolution(RESOLUTION_DJUI)
        mxui_hud_set_font(FONT_USER)
        mxui_hud_set_color(255, 255, 255, 255)
        mxui_hud_print_text(TEXT_PAUSE_UNAVAILABLE, width - 20, 16, 1)
    end

    -- Cross Fade to Menu
    mxui_hud_set_resolution(RESOLUTION_N64)
    mxui_hud_set_color(0, 0, 0, (math.abs(menuCrossFade)) * -menuCrossFadeMath)
    mxui_hud_render_rect(0, 0, width, height)
end
...
local function before_mario_update(m)
    ...
    if not startup_init_ready() then return end
    ...
    if dxUiOpen then
        ...
        local closeHeld = mod_bind_down(menuOpenBind) or (controller.buttonDown & (B_BUTTON | START_BUTTON)) ~= 0
        if dxUiIgnoreCloseUntilRelease then
            if not closeHeld then
                dxUiIgnoreCloseUntilRelease = false
            end
            dxUiStartupFrames = dxUiStartupFrames + 1
            nullify_inputs(m)
        ...
        dxUiStartupFrames = dxUiStartupFrames + 1
        if dxUiStartupFrames > 2 and (mod_bind_pressed(menuOpenBind) or (controller.buttonPressed & (B_BUTTON | START_BUTTON)) ~= 0) then
            character_dx_close_browser(false)
            return
        end
        nullify_inputs(m)
    ...
    if menuQueuedFromManager then
        if dx_is_game_menu_open() then
            dx_close_game_menu()
            nullify_inputs(m)
            return
        end
        if menuQueuedFromManagerFrames > 0 then
            menuQueuedFromManagerFrames = menuQueuedFromManagerFrames - 1
        else
            menuQueuedFromManager = false
            local queuedTab = dxUiQueuedTab or 1
            dxUiQueuedTab = 1
            if not character_dx_open_browser() then
                return
            end
            dxUiTab = math.clamp(queuedTab, 1, 4)
            return
        end
    end

    if CHARACTER_SELECT_DX_ONLY then
        local localPlayer = gCSPlayers[m.playerIndex]
        if menu or menuAndTransition or (localPlayer ~= nil and localPlayer.inMenu) then
            character_dx_clear_legacy_runtime(m, localPlayer, true)
        end
        if dx_is_game_menu_open() then
            return
        end
        if m.action ~= ACT_EXIT_LAND_SAVE_DIALOG and mod_bind_pressed(menuOpenBind) then
            open_character_select_dx()
            return
        end
        return
    end
    ...
    if dx_is_game_menu_open() then
        force_close_character_select_menu(m, gCSPlayers[m.playerIndex])
        return
    end

    if menu or (menuAndTransition and not dxUiOpen) then
        force_close_character_select_menu(m, gCSPlayers[m.playerIndex])
        return
    end

    if character == nil or character[character.currAlt or 1] == nil then
        update_character_render_table()
        character = characterTable[currChar]
        if character == nil then
            return
        end
    end

    if not menu and m.action ~= ACT_EXIT_LAND_SAVE_DIALOG and mod_bind_pressed(menuOpenBind) then
        open_character_select_dx()
        return
    end

    if not menuAndTransition and not menu then
        return
    end
    ...
    mouseScroll = mouseScroll - mxui_hud_get_mouse_scroll_y()
    ...
    -- Checks
    if characterTableRender[currCharRender] ~= nil then
...
hook_event(HOOK_ON_HUD_RENDER, function ()
    local ok, result = pcall(on_hud_render)
    if ok then
        return result
    end
    ...
    menuQueuedFromManager = false
    menuQueuedFromManagerFrames = 0
    pcall(force_close_character_select_menu, gMarioStates[0], gCSPlayers[0])
    pcall(cs_popup, "Character Select DX browser failed to render.", 2)
    print("[Character Select DX] on_hud_render failed: " .. tostring(result))
end)
...
local function chat_command(msg)
    ...
    if (msg == "" or msg == "menu") then
        if menu_is_allowed(gMarioStates[0]) then
            ...
            cs_notice(TEXT_PAUSE_UNAVAILABLE, 2)
            return true
        end
    end
    ...
    if msg == "?" or msg == "help" then
        cs_notice("Character Select DX commands are disabled in SM64DX.\nOpen the menu from \\#ffff33\\Player > Open Character Select\\#ffffff\\ or use the hotkey from Mod Controls.", 4)
        return true
    end
    ...
    if not menu_is_allowed() or charBeingSet then
        cs_notice("Character Cannot be Changed", 2)
        return true
    end
        ...
        if characterTable[i].locked ~= LOCKED_TRUE then
            ...
            for a = 1, #characterTable[i] do
                if msg == string.lower(characterTable[i][a].name) or msg == saveName then
                    ...
                    cs_notice('Character set to "' .. characterTable[i][characterTable[i].currAlt].name .. '" Successfully!', 2)
                    return true
                end
            end
        end
    ...
    if tonumber(msgSplit[1]) then
        ...
        if charNum > 0 and charNum <= #characterTable and characterTable[charNum].locked ~= LOCKED_TRUE then
            ...
            cs_notice('Character set to "' .. characterTable[charNum][altNum].name .. '" Successfully!', 2)
            return true
        end
    end

    cs_notice("Character Not Found", 2)
    return true
end

...
static int djui_panel_player_target_slot(void) {
    if (gDjuiInMainMenu) {
        return configHostSaveSlot;
    }
    return gCurrSaveFileNum;
}

static void djui_panel_player_open_moonos(struct DjuiBase* caller) {
    configHostSaveSlot = djui_panel_player_target_slot();
...
static void djui_panel_player_edit_palette_destroy(struct DjuiBase* caller) {
    ...
    sm64dx_capture_current_setup(djui_panel_player_target_slot());
...
static void djui_panel_player_append_feature(char *buffer, size_t bufferSize, const char *label) {
    if (buffer == NULL || bufferSize == 0 || label == NULL || label[0] == '\0') {
        return;
    }
...
static void djui_panel_player_add_moonos_summary(struct DjuiBase *body) {
    int slot = djui_panel_player_target_slot();
    int packIndex = sm64dx_get_selected_moonos_pack_index(slot);
    const struct Sm64dxMoonosPack *pack = (packIndex >= 0) ? sm64dx_get_moonos_pack(packIndex) : NULL;
    char titleLine[128] = { 0 };
    char sourceLine[160] = { 0 };
    char featureLine[160] = { 0 };
    const char *sourceName = "Native";

    snprintf(titleLine, sizeof(titleLine), "MoonOS Pack: %s", sm64dx_get_save_pack_name(slot));

    if (pack != NULL) {
        if (pack->sourceType == SM64DX_MOONOS_SOURCE_DYNOS) {
            sourceName = "DynOS";
        } else if (pack->hasLuaScript) {
            sourceName = "MoonOS";
        }

        snprintf(sourceLine, sizeof(sourceLine), "Source: %s | Base %s | %s",
                 sourceName,
                 pack->baseCharacter[0] != '\0' ? pack->baseCharacter : "Mario",
                 pack->compatibility[0] != '\0' ? pack->compatibility : "Compatible");

        if (pack->hasLuaScript) { djui_panel_player_append_feature(featureLine, sizeof(featureLine), "Lua"); }
        if (pack->hasDynosAssets) { djui_panel_player_append_feature(featureLine, sizeof(featureLine), "DynOS"); }
        if (pack->hasVoices) { djui_panel_player_append_feature(featureLine, sizeof(featureLine), "Voices"); }
        if (pack->hasLifeIcon) { djui_panel_player_append_feature(featureLine, sizeof(featureLine), "Life Icon"); }
        if (pack->hasHealthMeter) { djui_panel_player_append_feature(featureLine, sizeof(featureLine), "Life Meter"); }
        if (pack->hasMoveset) { djui_panel_player_append_feature(featureLine, sizeof(featureLine), "Moveset"); }
        if (pack->hasAnimations) { djui_panel_player_append_feature(featureLine, sizeof(featureLine), "Animations"); }
        if (pack->hasAttacks) { djui_panel_player_append_feature(featureLine, sizeof(featureLine), "Attacks"); }
    } else {
        snprintf(sourceLine, sizeof(sourceLine), "Source: Native | Base %s | Save-bound profile",
                 configPlayerModel < CT_MAX ? gCharacters[configPlayerModel].name : "Mario");
    }

    if (featureLine[0] == '\0') {
        snprintf(featureLine, sizeof(featureLine), "Use Browse MoonOS to switch native, MoonOS, and DynOS character setups.");
    }

    struct DjuiText* moonosTitle = djui_text_create(body, titleLine);
    djui_text_set_alignment(moonosTitle, DJUI_HALIGN_CENTER, DJUI_VALIGN_TOP);
    djui_base_set_size_type(&moonosTitle->base, DJUI_SVT_RELATIVE, DJUI_SVT_ABSOLUTE);
    djui_base_set_size(&moonosTitle->base, 1.0f, 26);
    djui_base_set_color(&moonosTitle->base, 200, 235, 255, 255);
    djui_text_set_drop_shadow(moonosTitle, 64, 64, 64, 100);

    struct DjuiText* sourceText = djui_text_create(body, sourceLine);
    djui_text_set_alignment(sourceText, DJUI_HALIGN_CENTER, DJUI_VALIGN_TOP);
    djui_base_set_size_type(&sourceText->base, DJUI_SVT_RELATIVE, DJUI_SVT_ABSOLUTE);
    djui_base_set_size(&sourceText->base, 1.0f, 24);
    djui_base_set_color(&sourceText->base, 220, 220, 220, 255);
    djui_text_set_drop_shadow(sourceText, 64, 64, 64, 100);
    djui_text_set_font_scale(sourceText, gDjuiFonts[0]->defaultFontScale * 0.50f);

    struct DjuiText* featureText = djui_text_create(body, featureLine);
    djui_text_set_alignment(featureText, DJUI_HALIGN_CENTER, DJUI_VALIGN_TOP);
    djui_base_set_size_type(&featureText->base, DJUI_SVT_RELATIVE, DJUI_SVT_ABSOLUTE);
    djui_base_set_size(&featureText->base, 1.0f, 36);
    djui_base_set_color(&featureText->base, 180, 235, 185, 255);
    djui_text_set_drop_shadow(featureText, 64, 64, 64, 100);
    djui_text_set_font_scale(featureText, gDjuiFonts[0]->defaultFontScale * 0.48f);
}
...
void djui_panel_player_create(struct DjuiBase* caller) {
    ...
    struct DjuiBase* body = djui_three_panel_get_body(panel);
    {
        ...
        djui_panel_player_add_moonos_summary(body);
        djui_button_create(body, "Browse MoonOS", DJUI_BUTTON_STYLE_NORMAL, djui_panel_player_open_moonos);

...
static void djui_panel_main_continue_last(UNUSED struct DjuiBase* caller) {
...
void djui_panel_main_create(struct DjuiBase* caller) {
    struct DjuiThreePanel* panel = djui_panel_menu_create(configExCoopTheme ? "\\#ff0800\\SM\\#1be700\\64\\#00b3ff\\EX\n\\#ffef00\\COOP" : "", false);
    {
        struct DjuiBase* body = djui_three_panel_get_body(panel);
        {
            ...
            struct DjuiButton* button1 = djui_button_create(body, "Start", DJUI_BUTTON_STYLE_NORMAL, djui_panel_start_create);
            ...
            if (sm64dx_has_recent_save()) {
                struct DjuiButton* recentButton = djui_button_create(body, "Continue Last File", DJUI_BUTTON_STYLE_NORMAL, djui_panel_main_continue_last);
                if (!configExCoopTheme) { djui_base_set_location(&recentButton->base, 0, -30); }
            }
